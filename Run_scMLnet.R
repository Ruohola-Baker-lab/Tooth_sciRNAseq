...
getHighExpGene <- function(GCMat,barCluTable,CluNum.1,CluNum.2,pval,logfc,cores)
{
  ...
  #saveRDS(GenePval, paste(RecClu, age,"enriched_full_clusters_FC.rds", sep="_"))
...
TFTargetFisher <- function(TFTableFile,Tup,TotalGene)
{
  ...
  FinalTable <- tempTable[which(as.vector(tempTable$pval) < 0.15),]
...
getCorR <- function(GCMat,keys,cells)
{
  ...
  cor_df <- cor_df[ cor_df[,1] > 0 &  cor_df[,2] < 0.15,]
...
getBarList <- function(Aclu,Bclu,barcluTable)
{
  ...
  AcluBar <- as.vector(barcluTable$Barcode[which(barcluTable$Cluster %in% Aclu)])
  BcluBar <- as.vector(barcluTable$Barcode[which(barcluTable$Cluster %in% Bclu)])
...
#'
#' @param GCMat scRNA-seq data. The gene expression matrixï¼raw) with rows as genes (gene symbols) and columns as cells.
                     ...
                     age)
{
  ...
  #saveRDS(RecClus, paste(RecClu, age,"enriched_full_clusters.rds", sep="_"))
  TFTargetTableResult <- TFTargetFisher(TFTableFile,filtered,TotalGene)
