...
DrawMLnet <- function(MLnetList,LigClu,RecClu,outdir = NULL,PyHome,plotMLnet = F){
  ...
  if(!file.exists("top_path_output")){ dir.create("top_path_output") }
  i <- paste(strsplit(LigClu,split = "\\W")[[1]][1],strsplit(RecClu,split = "\\W")[[1]][1],sep = "_")
  if(is.null(outdir)){
    wd <- paste("./top_path_output",i,sep = "/")
    if(!file.exists(wd)){dir.create(wd)}
  }else{
    wd <- paste("./top_path_output",outdir,sep = "/")
    if(!file.exists(wd)){dir.create(wd)}
    wd <- paste("./top_path_output",outdir,i,sep = "/")
    if(!file.exists(wd)){dir.create(wd)}
  }
