...
cds_base <- readRDS("data_sci_RNA_seq/OE_Am_cds_may22.rds") # replace mixed_cds with your cds name 


colData(cds_base)$clusters_joined <- dplyr::recode_factor(colData(cds_base)$clusters_named,
                                                          "diff_1" = "diff_1", 
                                                          "diff_2" = "diff_2", 
                                                          "diff_3" = "diff_3_4",
                                                          "diff_4" = "diff_3_4",
...
clustering_column <- "clusters_joined"  #clusters_named
age_clustering_column <- "diff_time_point"


#define your time point in your data
age_list <-  c("day_10","day_16")
...
age_next_list <-  c("day_16","day_16")


# define your receiver cell of interest at each time point
receiver_cell <- list('day_10' = "diff_4",
                      'day_16' = "diff_6")

receiver_cell <- list('day_10' = "diff_3_4",
                      'day_16' = "diff_5_6")

receiver_cell <- list('day_16' = "diff_7")

# # I have separate analysis for 20_22w
# receiver_cell <- list('20_22w' = "AM-1")

# define your differentiation target cell type at each time point of analysis
diff_target_cell <- list('day_10' = "diff_5_6",
                         'day_16' = "diff_8")

diff_target_cell <- list('day_10' = "diff_10",
                         'day_16' = "diff_7")

diff_target_cell <- list('day_16' = "diff_8")
diff_target_cell <- list('day_16' = "diff_9")

# # I have separate analysis for 20_22w
# diff_target_cell <- list('20_22w' = "AM-2")

# Define the group of cells that are present at each time point
in_groups <- list('day_10' = c("diff_1" ,"diff_2",  "diff_3", "diff_4","diff_12","diff_13","diff_15"),
                  'day_16' = c("diff_10", "diff_5", "diff_11" , "diff_9","diff_6","diff_7","diff_8","diff_14"))

in_groups <- list('day_10' = c("diff_1" ,"diff_2",  "diff_3_4","diff_12","diff_13","diff_15"),
                  'day_16' = c("diff_10", "diff_5_6", "diff_11" , "diff_9","diff_7","diff_8","diff_14"))
...
cores = ifelse(cores_num>1,cores_num-34,1)
param <- MulticoreParam(workers = cores, progressbar = TRUE)
register(param)
...
for (pair in pair.list){
  ...
  results <- DEsingle(counts = counts(temp_cds), group = group, parallel = T, BPPARAM =param)
...
for (age in age_list){
  ...
  RecClu <- unlist(c(receiver_cell[age]))    # the reciver "diff_3"
  LigClu <- setdiff(unlist(included), RecClu)     #all clusters except the recevier
  pval <- 0.05
  logfc <-0.15 #0.15  note it make casue an error saying incorrect number of dimension when only one gene pass
  #get cores
  ...
  saveRDS(RecClus, paste(meta.file,"/",paste(RecClu, collapse = "_"),"_", age,"_enriched.rds", sep=""))
...
# CluNum.2 <- LigClu
# GCMat,barCluTable,CluNum.1,CluNum.2,pval,logfc,cores)
